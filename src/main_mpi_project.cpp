...
int main(int argc, char **argv)
{
    ...
    //How to derive the 2D-process number from the 1D-MPI-rank:
    //Paritions grid to processes
    ...
    // boundary cell initialization and calculation

    bool isLeftBoundaryCell = false; // conditional syntax was strange with false negatives
    bool isRightBoundaryCell = false;
    bool isBottomBoundaryCell = false;
    bool isTopBoundaryCell = false;

    //I believe this is fully generalized
    //Set right-left boundary
    if (myRank % numPartsY == 0){
        isLeftBoundaryCell = true;
    }
    if(myRank % numPartsY == (numPartsY-1)){
        isRightBoundaryCell = true;
    }

    //set top-bottom boundary
    if(myRank < numPartsY){
        isTopBoundaryCell = true;
    }
    if(myRank >= ((numPartsX-1)*numPartsY)){
